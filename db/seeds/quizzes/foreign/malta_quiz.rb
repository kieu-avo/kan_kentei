...
#---------問題1--------------------------------------------------------------------★
quiz1 = Quiz.create!(
  content: "①これからマルタにバカンスに行きます。飛行機で出会った超絶美人なイタリア人女性から、『マルタ』は実は正式名称ではないことを聞きました。正式名称は何でしょう？",
  youtube_url: nil,
  image_url: nil,
  test_category: malta
)
...
#---------問題6--------------------------------------------------------------------★
quiz6 = Quiz.create!(
  content: "⑥一緒にランチをしよう！と食事に誘われました（とても嬉しいけど、緊張して喉が通るか心配です）。マルタではどんな料理が有名でしょう？",
  youtube_url: nil,
  image_url: nil,
  test_category: malta
)
